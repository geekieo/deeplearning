...
class Node(object):
    ...
    def append_forwardstream_connection(self, conn):
        '''
        添加一个到前一层节点的链接
        '''
        self.f_stream.append(conn)

    def append_backstream_connection(self, conn):
        '''
        添加一个到后一层节点的链接
        '''
    ...
    def calc_hidden_layer_error(self):
        '''
        计算隐层节点误差梯度
        隐层BP公式：error = output*(1-output)*Σ(W·out_errors)
            error 为当前节点方向传播误差值；output 为当前节点前馈输出值；
            W 为当前节点前馈权重，多个权重；out_errors 为当前节点前馈节点误差，多个误差；
